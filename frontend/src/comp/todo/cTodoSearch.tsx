import { useState, useEffect } from 'react'; 
import { useRecoilState } from 'recoil';
import { yupResolver } from '@hookform/resolvers/yup';
import yup from '../../yup/yup.jp';
import { useForm } from 'react-hook-form';
import { todoSearchWordAtom } from '../../state/atom'; 

const schema = yup.object({
    searchWord: yup.string().label('検索項目').allowAlphaNumJapan()
});

export default function CompTodoSearch() {

    const [ inputValue, setInputValue ] = useState('')
    const [ searchWord, setSearchWord] = useRecoilState(todoSearchWordAtom);

    const { register, handleSubmit, formState: {errors} } = useForm({
        defaultValues: {
            searchWord: ''
        },
        resolver: yupResolver(schema),
    });

    useEffect (() => {
        if( searchWord == "") {
            setInputValue("");
        }
    },[searchWord]);

    const onsubmit = async (target: any) => {
        setSearchWord(target.searchWord);
    };

    const onerror = (err:any) => console.log(err);

    return (
        <div className="box-first">
            <h2>Search</h2>
            <form className="diarySearch" onSubmit={handleSubmit(onsubmit, onerror)}>
                <input id="searchWord" 
                       type="text"
                       autoComplete="off" 
                       value={inputValue}
                       {...register('searchWord')} 
                        onChange={(e) => {
                        setInputValue(e.target.value);
                        }} 
                 />
                <div className="right">
                    <button className="btn-min1">search</button>
                </div>
                <p className="alert">
                    {errors.searchWord?.message}
                </p>
            </form>
        </div>
    );
}